...
This single archive remains only so Skills installed at version `0.13.25` can complete their already-published one-time updater path. That older updater is hard-coded to this exact repository path. The archive contains the current source-only sync implementation and the six current Skills; it must not contain a Node runtime, a direct API executor, or retired monolithic model-selection files. After that migration, later updates use the repository-source flow. Do not link this file in user installation instructions or create a second payload archive.
