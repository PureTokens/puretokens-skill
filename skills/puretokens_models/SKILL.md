...
通过宿主当前已配置的连接只读查询当前可用的媒体模型。宿主负责 Base URL、认证、路由和 HTTP 执行；Skill 只请求相对路径，绝不读取、扫描、展示或索取凭据或配置，也不检查 Base URL、provider 标签或服务归属。

先读取已安装的 `references/execution-contract.json` 和 `references/behavior-scenarios.json`；它们定义模型目录、能力筛选和用户可见输出的约束。

## 查询规则

- 每个用户查询只调用一次 `GET /v1/media/models`；不得调用 Images/Videos 提交、任务状态、内容、余额或其他路径，也不得重试。
...
- 目录为空、无法读取或模型不在当前目录时，如实说明当前连接未返回对应模型资料，并提示用户检查已有 Pure Tokens Base URL、认证和路由配置；不切换路径、不尝试静态目录，也不索取凭据。
