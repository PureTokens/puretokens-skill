...
- 升级不能要求用户安装 Node、npm、包管理器、Git 或开发环境。每次更新都先把紧凑官方安装包下载到新的私有临时目录：优先 GitHub 官方 API raw-content 路径，失败时只允许 GitHub raw-content 再尝试一次。解压后直接执行其中最新匹配的平台安装器：macOS/Linux 使用 `puretokens-skill-install.sh sync --host <当前宿主 ID>`，Windows 使用 `puretokens-skill-install.ps1 sync -Host <当前宿主 ID>`。安装器自行推导该宿主的全局 Skill 目录并校验解压后的官方源目录。不得运行已安装的旧更新器、复用用户连接配置，或先下载 runtime 安装器再让它第二次下载安装包。不得把远程内容直接管道给 Shell/PowerShell，也不得退回 Node 流程。这两个官方平台安装器只使用操作系统自带的 HTTPS 下载、归档和文件操作能力；它们先完成只读静态校验，再在同一次运行中同步。每个下载路径最多 20 秒；两条路径都失败时停止，绝不使用第三方镜像、反复重试或开始同步。不得先运行 `node`、`npm`、Git、生成器或安装依赖。
