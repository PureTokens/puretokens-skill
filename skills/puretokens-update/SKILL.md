...
- 升级不能要求用户安装 Node、npm、包管理器、Git 或开发环境。每次更新都先从 GitHub 官方 API raw-content 路径下载最新匹配平台安装器到新的私有临时目录：macOS/Linux 使用 `puretokens-skill-install.sh`，Windows 使用 `puretokens-skill-install.ps1`；随后执行该本地脚本。仅当 API 路径失败时，才允许从 GitHub raw-content 路径再下载一次。不得继续使用已安装的旧安装器，因为它可能缺少当前的下载兼容性修复。不得把远程内容直接管道给 Shell/PowerShell，也不得退回 Node 流程。这两个官方平台安装器只使用操作系统自带的 HTTPS 下载、归档和文件操作能力；它们下载不含文档和品牌资源的紧凑官方安装载荷，先完成只读静态载荷校验，再在同一次运行中同步。每个下载路径最多 20 秒；两条路径都失败时停止，绝不使用第三方镜像、反复重试或开始同步。不得先运行 `node`、`npm`、Git、生成器或安装依赖。
