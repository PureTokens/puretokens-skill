...
```json
{"kind":"image","original_operation":"generate","task_id":"RETURNED_ID","model":"gpt-image-2","requested_count":1,"parameters":{"image_size":"1K"}}
```
...
For multiple images, preserve the accepted count and retrieve indexes 0..n-1. Attach the returned `downloaded_paths` file before requesting another index. Downloading is not delivery. Existing validated same-task files in the same directory are reused.
