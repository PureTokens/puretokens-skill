...
- 提交获得 `task_id` 后才开始自动轮询。若状态响应有有效的 HTTP `Retry-After` 秒数，优先按它等待；否则依次等待 2、3、5、8 秒，之后每 15 秒查询一次。自动轮询从提交响应起最多 300 秒；到期仍为 pending/running 时，报告任务仍在处理中和当前状态，停止自动等待，要求用户明确要求继续查询同一 `task_id`。不得把轮询到期当作失败或重提理由。
