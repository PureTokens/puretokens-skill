...
- Computer Use, browser automation, desktop/mobile UI automation, and opening or clicking Pure Tokens Switch/Desktop are never API transports. Do not use them to discover a connection, obtain a credential, submit a request, poll a task, retrieve media, or recover from a missing host-native API executor.
- API-facing Skills must not invoke Imagen or another image/video Skill as a fallback. If the current session does not expose host-native authenticated HTTPS/API execution, stop before any request with the documented validation failure; never search for a visible generation interface or request Computer Use permission.
