...
- Image edit: `Use grok-imagine-image to edit the attached image: replace the cloudy sky with a clear sunset.` The Skill uses the installed declared image-edit operation, or reads the catalog once only if that operation is absent from the installed selection.
...
This list is generated from Pure Tokens' base model catalog using explicit image/video capabilities. Normal generation uses the installed versioned selection directly; GET /v1/media/models is an on-demand lookup only for explicit discovery, an installed-profile gap, or post-rejection diagnosis.
