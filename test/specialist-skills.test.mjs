...
test("bilingual READMEs include safe copyable Agent installation prompts", async () => {
  ...
  assert.equal((english.match(/<!-- agent-install-prompt:start -->/g) ?? []).length, 1);
  assert.equal((english.match(/<!-- agent-install-prompt:end -->/g) ?? []).length, 1);
  const englishPrompt = english.match(/<!-- agent-install-prompt:start -->\s*```text\n([\s\S]*?)\n```\s*<!-- agent-install-prompt:end -->/);
  assert.ok(englishPrompt, "English client-download prompt must be one bounded text block");
  assert.match(english, /current stable Pure Tokens Skills from the main branch/);
  assert.match(english, /\$env:USERPROFILE\\\.agents\\skills/);
  assert.match(english, /~\/\.agents\/skills on macOS\/Linux/);
  assert.match(english, /Claude Code uses ~\/\.claude\/skills; Gemini CLI uses ~\/\.gemini\/skills/);
  ...
  assert.match(english, /contains SKILL\.md and skill\.json and that every manifest name matches its directory name/);
  ...
  assert.equal((chinese.match(/<!-- agent-install-prompt:start -->/g) ?? []).length, 1);
  assert.equal((chinese.match(/<!-- agent-install-prompt:end -->/g) ?? []).length, 1);
  const chinesePrompt = chinese.match(/<!-- agent-install-prompt:start -->\s*```text\n([\s\S]*?)\n```\s*<!-- agent-install-prompt:end -->/);
  assert.ok(chinesePrompt, "Chinese client-download prompt must be one bounded text block");
  assert.match(chinese, /main 分支上的稳定版 Pure Tokens Skills/);
  ...
  assert.match(chinese, /macOS\/Linux 上的 Codex 使用 ~\/\.agents\/skills/);
  assert.match(chinese, /Claude Code 使用 ~\/\.claude\/skills；Gemini CLI 使用 ~\/\.gemini\/skills/);
